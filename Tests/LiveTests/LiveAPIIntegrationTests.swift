...
struct LiveAPIIntegrationTests {
    ...
    func testLiveDatabaseReleaseFetch() async throws {
        ...
        print("🔍 Testing live release fetch for Abbey Road (ID: 1362270)...")
        
        // Abbey Road by The Beatles - a famous release
        let release = try await client.database.getRelease(id: 1362270)
        
        // Verify basic release properties
        #expect(release.id == 1362270)
        #expect(release.title.contains("Abbey Road"))
    ...
    @Test("Live Database - Fetch Label (Apple Records)")
    func testLiveDatabaseLabelFetch() async throws {
        guard let client = createLiveClient() else {
            throw TestSkipError("No valid API token provided")
        }
        
        print("🔍 Testing live label fetch for Apple Records (ID: 20871)...")
        
        // Apple Records - The Beatles' label
        let label = try await client.database.getLabel(id: 20871)
        
        // Verify basic label properties
        #expect(label.id == 20871)
        #expect(label.name.contains("Apple"))
        #expect(label.profile?.isEmpty == false)
    ...
    func testLiveRateLimiting() async throws {
        ...
        let artistIds = [82730, 45, 1, 194, 1000] // Various famous artists
